...
def profile(request):
    ...
    follower_list = [
        follower.follower.username for follower in Follow.objects.filter(following=user)]
    following_list = [
        following.following.username for following in Follow.objects.filter(follower=user)]
    post_form = PostForm()
...
def user(request, username):
    ...
    follower_list = [follower.follower.username for follower in Follow.objects.filter(
        following=user_other)]
    following_list = [following.following.username for following in Follow.objects.filter(
        follower=user_other)]
    liked_post = Like.objects.filter(user=request.user)
    liked_post_list = liked_post.values_list('post', flat=True)
    ...
    context = {
        ...
        'liked_post_list': liked_post_list,
        'title': user_other.username,
        'already_followed': already_followed,
        'follower_list': follower_list,
        'following_list': following_list,
        **base_context(request)
    }
    return render(request, 'App_Login/other_user.html', context)
...
def follow(request, username):
    ...
    if not already_followed:
        ...
        notification = Notification(
            user=following_user, notification_type='Follow', target=follower_user)
        notification.save()
